...
import { f as filtered, t as toUpperCase } from "../../chunks/utils.js";
function _page($$payload, $$props) {
  push();
  let { data } = $$props;
  const subSet = data.pokemon.slice(0, 150);
  let val = "";
  const each_array = ensure_array_like(filtered(val, subSet));
  $$payload.out += `<h1>All Pokemon</h1> <input type="text" placeholder="Search..."${attr("value", val)}> <ul class="svelte-5zrsop"><!--[-->`;
